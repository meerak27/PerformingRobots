...
// MEERA EDIT added case 1-2 and updateedflashes
void loop() {
  ...
  if (radio.available(&pipeNum)) {
    ...
    switch (data.stateNumber) {
      ...
      case 0:
        ...
        musicPlayer.startPlayingFile("/machines.mp3");
      ...
      case 1: 
        ...
        // Turn LEDs red and start the timer
        setColor(125, 0, 125); // purple
        isFlashingRainbow = false;
        isFlashingRed = true;
        redFlashStartTime = millis();
        isCrankPulling = false;
      ...
      case 2:
    //     Serial.println(F("Flashing rnbw and playing aside"));
    //     // Stop red flash if it's running
    //     if (isCrankPulling) {
    //       // If we get a "0" command but are already pulling, ignore it.
    //       Serial.println(F("...but crank is already pulling. Command ignored."));
    //     } else {
    ...
    //     lastRainbowUpdate = millis();
    //     rainbowOffset = 0; // Reset rainbow position
  ...
  if (isCrankPulling) {
    ...
    // >>> NEW: attach servo before starting motion <<<
    if (!crank.attached()) {
      crank.attach(CRANK_SERVO_PIN);
    }
